    ...
    if ($result) {
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td class='text-right' style='color: white'><button class='editBtn btn btn-info'  data-id='" . $row["id"] . "'><img class='fa w-6 h-6' src='assets/svg/pencil.svg' alt=''></button>
             <button class='deleteBtn btn btn-success' style='color: white' data-id='" . $row["id"] . "'><img class='fa w-6 h-6' src='assets/svg/trash.svg' alt=''></button></td>";
                echo "</tr>";
            }
        } else {
